...
class GraphicUserInterface(UserInterface):
...
def main():
    ...
    while True:
        ...
        if user_input in ["good bye", "close", "exit"]:
            interface.display_message(handle_save(current_directory))
            interface.display_message("Good bye!")
            break
        for command in COMMANDS.keys():
            if user_input.startswith(command):
                args = user_input[len(command):].split()
                res = COMMANDS[command](*args)
                interface.display_message(res) if res is not None else ...
                break
        else:
            interface.display_message("Unknown command. Please try again.")
